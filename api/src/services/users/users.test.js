...
describe('users', () => {
  ...
  scenario('creates a user', async (scenario) => {
    const result = await createUser({
      input: {
        ...
        name: 'String',
        enlisted: true,
        issueTemplateId: scenario.user.two.issueTemplateId,
      },
    })

    expect(result.email).toEqual('String9624488')
    expect(result.salt).toEqual('String')
    expect(result.hash).toEqual('String')
    expect(result.hashedPassword).toEqual('String')
    expect(result.name).toEqual('String')
    expect(result.enlisted).toEqual(true)
    expect(result.issueTemplateId).toEqual(scenario.user.two.issueTemplateId)
  })
  ...
  scenario('updates a user', async (scenario) => {
    ...
    const result = await updateUser({
      id: original.id,
      input: { email: 'String77729522' },
    })

    expect(result.email).toEqual('String77729522')
  })
